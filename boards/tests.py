...
class NewTopicTests(TestCase):
    ...
    def test_new_topic_viw_not_found_status_code(self):
        url = reverse('new_topic', kwargs={'pk': 100})
        response = self.client.get(url)
        self.assertEquals(response.status_code, 404)

    def test_new_topicurl_resolves_new_topic_view(self):
        new_topic_view = resolve('/boards/1/new/')
        self.assertEquals(new_topic_view.func, new_topic)
    ...
    def test_new_topic_invalid_post_data_empty_fields(self):
        ...
        self.assertEquals(Post.objects.exists())